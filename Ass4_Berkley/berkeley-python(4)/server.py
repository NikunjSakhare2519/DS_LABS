...
def synchronizeAllClocks():
    while True:
        try:
            ...
            if len(client_data) > 0:
                # Calculate the average clock difference across all clients
                average_clock_difference = getAverageClockDiff()

                # Send synchronized time to each client
                for client_addr, client in client_data.items():
                    try:
                        # Synchronized time based on the average difference
                        synchronized_time = datetime.datetime.now() + average_clock_difference
                        client['connector'].send(str(synchronized_time).encode())
                        print(f"Synchronized time sent to {client_addr}")
                    except Exception as e:
            ...
            print(f"Error in synchronization cycle: {e}")
            time.sleep(5)
